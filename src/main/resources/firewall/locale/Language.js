...
Ext.define('Stackops.portal.plugin.firewall.locale.Language', {
	languages : {
		es : {
			...
			'<table>' + '<tr> ' + '<td width=50></td>' + '<td width=100><font size=1pt>Admin State Up:</td>' + '<td><font size=1pt>$1</p></td>' + 
			'<td width=100></td>' + '<td width=150> <font size=1pt> Política de corta fuegos </td>' + '<td> <font size=1pt> $2 </td>' + '</tr>' + 
			'<tr>' + '<td width=50></td>' + '<td width=50 ><font size=1pt>Estado:</td>' + '<td><font size=1pt> $3 </p></td>' + '<td width=100></td>' + 
			'<td width=75> <font size=1pt> Id:   </td>' + '<td> <font size=1pt> $4 </td>' + '</tr>' + '</table>',

			'policy-container-html' : '<img align = "left" style="padding-right: 10px;" src="plugin/static/firewall/images/policy_32.png" >' + 
			'<table>' + '<tr> ' + '<td width=50></td>' + '<td width=100><font size=1pt>Compartido:</td>' + '<td><font size=1pt>$1</p></td>' + 
			'<td width=100></td>' + '<td width=45> <font size=1pt> Id:   </td>' + '<td> <font size=1pt> $3 </td>' + '</tr>' + '<tr>' + '<td width=50></td>' + 
			'<td width=50 ><font size=1pt>Auditado:</td>' + '<td><font size=1pt>$2</p></td>' + '</tr>' + '</table>',
			...
			'\r\t· ICMP: Es usado por dispositivos de red, ' + '\r\t  tales como los ruters para enviar' + 
			'\r\t  mensajes de error. ICMP también' + '\r\t  puede ser usado para transmitir'+ '\r\t mensajes ICMP.\n' + 
			...
			'\r\t  se haya establecido previamente' + 
			'\r\t  conexión, tampoco tiene confirmación' + 
			'\r\t   ni control de flujo.\n' + 
			...
			'rule-enable-error' : 'Error habilitando la regla $1',
			'rule-disable-error' : 'Error deshabilitando la regla $1',
			...
			'fwaas-action-create-policy-shared' : 'Compartida',
			'fwaas-action-create-policy-audited' : 'Auditada',
			'fwaas-action-edit-policy-rule' : 'Reglas',
			'fwaas-action-edit-policy-name' : 'Nombre',
			'fwaas-action-edit-policy-description' : 'Descripción',
			'fwaas-action-edit-policy-shared' : 'Compartida',
			'fwaas-action-edit-policy-audited' : 'Auditada',
			...
			'fwaas-insert-rule' : 'Añadir Regla',
		...
		en : {
			...
			'firewall-container-html' : '<img align = "left" style="padding-right: 10px;" src="plugin/static/firewall/images/fw_32.png" >' + 
			'<table>' + '<tr> ' + '<td width=50></td>' + '<td width=100><font size=1pt>Admin State Up:</td>' + 
			'<td><font size=1pt>$1</p></td>' + '<td width=100></td>' + '<td width=75> <font size=1pt> Firewall Policy:   </td>' + 
			'<td> <font size=1pt> $2 </td>' + '</tr>' + '<tr>' + '<td width=50></td>' + '<td width=50 ><font size=1pt>Status:</td>' + 
			'<td><font size=1pt> $3 </p></td>' + '<td width=100></td>' + '<td width=75> <font size=1pt> Id:   </td>' + 
			'<td> <font size=1pt> $4 </td>' + '</tr>' + '</table>',

			'policy-container-html' : '<img align = "left" style="padding-right: 10px;" src="plugin/static/firewall/images/policy_32.png" >' + 
			'<table>' + '<tr> ' + '<td width=50></td>' + '<td width=100><font size=1pt>Shared:</td>' + '<td><font size=1pt>$1</p></td>' + '<td width=100></td>' + 
			'<td width=45> <font size=1pt> Id:   </td>' + '<td> <font size=1pt> $3 </td>' + '</tr>' + '<tr>' + 
			'<td width=50></td>' + '<td width=50 ><font size=1pt>Audited:</td>' + '<td><font size=1pt>$2</p></td>' + '</tr>' + '</table>',
			...
			'\r· Name: Choose a name that will help you to find the rule. ' + 'You can repeat existing names, so be careful.\n' + 
			'\r· Protocol: Here the user can specify the IP protocol to aply. These values can be:\n' + 
			'\r\t· ICMP: It is used by network devices, ' + '\r\t  like routers, to send error messages.' + 
			...
			'rule-enable-error' : 'Error enabling the rule $1',
			'rule-disable-error' : 'Error disabling the rule $1',

			'fwaas-create-firewall' : 'Create Firewall',
			'fwaas-create-policy' : 'Create Policy',
			'fwaas-create-rule' : 'Create Rule',
